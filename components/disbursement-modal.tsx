    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-[540px] max-h-[90vh] overflow-y-auto p-0 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="rounded-xl border p-4" style={{ background: "#FEFCE8", borderColor: "#FDE047" }}>
            <div className="flex items-start justify-between">
              ...
              <button className="text-yellow-600 hover:text-yellow-800 text-[18px] mt-1" title="Refresh">↻</button>
            </div>
          </div>
            ...
            <RadioGroup value={chargeType} onValueChange={(v) => setChargeType(v as ChargeType)} className="space-y-2">
              {charges.map((c) => (
                <label
                  ...
                  style={{
                    borderColor: chargeType === c.id ? "#16A34A" : "var(--border)",
                    background: chargeType === c.id ? "#F0FDF4" : "var(--muted)/0.3",
                  }}
                >
                  <div className="flex items-center gap-3">
                    <RadioGroupItem
                      value={c.id}
                      id={`charge-${c.id}`}
                      className="border-[#16A34A] data-[state=checked]:bg-[#16A34A] data-[state=checked]:text-white"
                    />
                    <div>
        ...
        <div className="px-6 py-4 border-t border-border">
          <Button
            onClick={onDisburse}
            className="w-full bg-[#16A34A] hover:bg-[#15803D] text-white h-11 text-[14px] font-bold"
          >
            Disburse funds
          </Button>
        </div>
      ...
      <DialogContent className="max-w-[360px] p-0" showCloseButton={false}>
        <div className="flex flex-col items-center justify-center px-8 py-12 text-center gap-5">
          <div
            ...
            style={{ borderTopColor: "#16A34A", animation: "spin 0.9s linear infinite" }}
          />
          <div>
          ...
          <button
            onClick={onSuccess}
            className="text-[12px] text-muted-foreground underline hover:text-foreground transition-colors mt-2"
          >
            Simulate success →
          </button>
        </div>
...
// STATE 3 — Success
// ---------------------------------------------------------------------------
function SuccessStep({ open, card, txId, onClose }: { open: boolean; card: FarmerRequest; txId: string; onClose: () => void }) {
  const totalAmount = card.farmers * AMOUNT_PER_FARMER;
  const now = new Date();
